...
def drawBoard(board):
    ...
    hline = '    ' # initial space for the numbers down the left side
    for i in range(1, 6):
        hline += (' ' * 9) + str(i)
...
def getNewBoard():
    ...
    board = []
    for x in range(60): # the main list is a list of 6- lists
        board.append([])
...
def enterPlayerMove():
    # Let the player type in his move.  Return a two-item list of int xy coordinates.
...
def playAgain():
    # This function returns True if the player wants to play again and...
...
while True:
	...
	# Start of a turn:
	# somar device/chest status
        if sonarDevices > 1: extraSsonar = 's'
        else: extraSsonar = ''
        if len(theChests) > 1: extraSchest = 's'
        else: estraSchest = ''
        print('You have %s sonar device%s letf. %s treasure chest%s remaining.' % (sonarDevices, extraSsonar, len(theChests), extraSchest))
        ...
        if len(theChests) == 0:
            print('You have found all the sunken treasure chests!  Congradulations and good game!')
            break
