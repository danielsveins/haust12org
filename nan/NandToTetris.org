...
*** status
status is on page 12 in chapter 4
*** Before Project
...
the first and second d-bits code weather to store the computed value in the A register and D register respectively.
...
The question is weather to jump.  If a jump occurs it is assumed that the address has already been put in the A register.
